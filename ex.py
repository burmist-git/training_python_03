...
def test_cros_dot_stack_expand_dims():
    ...
    rr = np.stack(np.expand_dims(np.array([np.array([i+j*nPoints+1. for i in range(nPoints)]) for j in range(4)]),axis=0))
    rrr = np.vstack(np.expand_dims(np.array([np.array([i+j*nPoints+1. for i in range(nPoints)]) for j in range(4)]),axis=0))
    rrrr = np.hstack(np.expand_dims(np.array([np.array([i+j*nPoints+1. for i in range(nPoints)]) for j in range(4)]),axis=0))
    ...
    print('len(rr)    = ',len(rr))
    print('len(rr[:]) = ',len(rr[:]))
    print('len(rr[0][:]) = ',len(rr[0][:]))
    print('len(rr[0][0][:]) = ',len(rr[0][0][:]))
    print('rr[0]      = ',rr[0])
    print('rr[0][0]      = ',rr[0][0])
    print('rr[0][1]      = ',rr[0][1])
    ...
    print('type(rrr)   = ', type(rrr))
    print('rrr         = ',rrr)
    print('len(rrr)    = ',len(rrr))
    print('len(rrr[:]) = ',len(rrr[:]))
    print('len(rrr[0][:]) = ',len(rrr[0][:]))
    ...
    print('type(rrrr)   = ', type(rrrr))
    print('rrrr         = ',rrrr)
    print('len(rrrr)    = ',len(rrrr))
    print('len(rrrr[:]) = ',len(rrrr[:]))
    print('len(rrrr[0][:]) = ',len(rrrr[0][:]))
